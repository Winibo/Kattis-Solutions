...
const Type oo = INT_MAX;      // Change to INT_MAX for min.
...
void constructRMQ(Type A[MAX_N], int n) {
  sizetemp = 1; while(sizetemp < n) sizetemp <<= 1;
  for (int i=0; i < sizetemp; i++) M[sizetemp-1+i] = (i < n ? make_pair(A[i],i) : p_oo);
  for (int i=sizetemp-2; i>=0; i--) M[i] = min(M[2*i+1],M[2*i+2]);
}

pti getmax(int a, int b, int st=0,int en=sizetemp,int ind=0) {
  if (a > b || a >= en || b < st)         return p_oo;
  if ((a <= st && en <= b) || st+1 >= en) return M[ind];
  int mid = (st+en)/2;
  return min(getmax(a,b,st,mid,2*ind+1), getmax(a,b,mid,en,2*ind+2));
}
...
void calculate(int index[], int sarray[], int length) {
...
int main() {
  ...
  constructRMQ(lcp, s.length());
  calculate(index, sarray, s.length());
  int queries;
  cin >> queries;
  for (int i = 0; i < queries; i++) {
    int k,j;
    cin >> k >> j;
    cout << getmax(min(index[k], index[j])+1,max(index[j], index[k])).first << endl;
  }
}
